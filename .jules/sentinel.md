...
## 2026-05-11 - Missing Subresource Integrity (SRI) on External CDN Scripts
**Vulnerability:** External scripts loaded from CDNs without SRI hashes are vulnerable to supply-chain attacks. If the CDN is compromised, malicious code could be injected.
**Learning:** Both static HTML files (`examples/dashboard/index.html`) and dynamic HTML generated by edge workers (`worker/index.js`) must include SRI hashes (`integrity` attribute) and `crossorigin="anonymous"` for all external scripts.
**Prevention:** Always verify external script tags have SRI hashes, especially when updating or adding new dependencies from CDNs.
